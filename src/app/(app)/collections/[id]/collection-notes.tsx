...
import { updateCollectionNotes } from "../actions";

const MAX = 5000;

/**
 * The collection's own notes («solo las de primera edición», «pedidas a Juan»…): shown as
 * written, line breaks and all, and edited in place. Longer than the description under the
 * title, which stays a one-line subtitle.
 */
export function CollectionNotes({ id, notes }: { id: string; notes: string | null }) {
  ...
  function save() {
    startTransition(async () => {
      try {
        await updateCollectionNotes(id, draft);
        setEditing(false);
    ...
    return (
      <section className="space-y-2" aria-label="Notas">
        <Textarea
          ...
          maxLength={MAX}
          autoFocus
          aria-label="Notas de la colección"
          placeholder="Lo que quieras apuntar de esta colección: qué versiones buscas, a quién se la has pedido…"
        />
